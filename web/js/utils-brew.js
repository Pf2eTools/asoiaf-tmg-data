...
class BrewDoc {
	...
	mutMerge ({json, isLazy = false}) {
		this.body = this.constructor.mergeObjects({isCopy: !isLazy}, this.body, json);
		this.head.mutMerge({json, body: this.body, isLazy});
		return this;
	}

	static mergeObjects ({isCopy = true} = {}, ...jsons) {
		const out = {};
...
class _BrewUtil2Base {
	...
	async _pGetBrew_pGetLocalBrew ({lockToken} = {}) {
		...
		if (IS_DEPLOYED || typeof window === "undefined") return this._cache_brewsLocal = [];
	...
	_PAGE_TO_PROPS = {
		[UrlUtil.PG_SONG]: [...UrlUtil.PAGE_TO_PROPS[UrlUtil.PG_SONG]],
	...
	_getBrewPage (page) {
		return page || UrlUtil.getCurrentPage();
	}
