...
globalThis.VeCt = {
	...
	JSON_PRERELEASE_INDEX: `../custom/index.json`,
	JSON_BREW_INDEX: `../custom/index.json`,
...
globalThis.UrlUtil = {
	...
	/**
	 * All internal URL construction should pass through here.
	 *
	 * @param href the link
...
UrlUtil.PG_SONG = "song.html";

UrlUtil.URL_TO_HASH_GENERIC = (it) => UrlUtil.encodeArrayForHash(it.id, it.lang);
...
globalThis.DataUtil = {
	...
	userDownloadBlob (filename, blob) {
		...
		async _pMergeCopy (impl, page, entryList, entry, options) {
			...
			if (!it) {
				if (options.isErrorOnMissing) {
					...
					if (!IS_DEPLOYED) throw new Error(`Could not find "${page}" entity "${entry._copy.name}" ("${entry._copy.source}") to copy in copier "${entry.name}" ("${entry.source}")`);
				}
				return;
			}
	...
	song: class extends _DataUtilPropConfigCustom {
		static _PAGE = UrlUtil.PG_SONG;
		static _BASEDIR = "../data";
		...
		static async pLoadSingleSource (source) {
			const abilityData = await DataUtil.loadJSON(`${this._BASEDIR}/${source}/abilities.json`);
			const data = await Parser.FACTIONS.pMap(async f =>
...
class BookModeViewBase {
	...
	async _$pGetWrpFooter () {
		return $(`<div class="w-100 ve-flex-vh-center no-shrink no-print"></div>`);
	}
	...
	async pOpen () {
		...
		document.title = `${this._pageTitle} - asoiaf:tmg`;
		...
		this._$wrpBook = $$`<div class="bkmv print__h-initial ve-flex-col print__ve-block">
			...
			${$wrpContentOuter}
			${await this._$pGetWrpFooter()}
		</div>`
			.appendTo(document.body);
	}

	async _pGetContentElementMetas () {
		const $wrpContent = $(`<div class="ve-flex-wrap smooth-scroll ve-overflow-y-auto print__overflow-visible w-100 min-h-0"></div>`);
